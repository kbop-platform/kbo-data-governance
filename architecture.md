...
flowchart TB
    ...
    subgraph REPO["kbo-data-governance 저장소"]
        ...
        MANUAL["standards/ governance/\nmigration/ catalog/\nstandards-dict/ glossary/"]
        XLSX["exports/\n데이터사전.xlsx"]
    end

    subgraph DEPLOY["GitHub Pages (정적 사이트)"]
        AUTH["auth.js\n인증 게이트"]
        CATALOG["Catalog\nAG Grid"]
...
```mermaid
flowchart TB
    subgraph MKDOCS["MkDocs Material"]
        MD["Markdown → HTML 변환"]
    end

    subgraph PAGES["Markdown 페이지"]
        DICT_MD["dictionary/*.md\n테이블 사전"]
        STD_MD["standards/*.md\n표준 문서"]
        GOV_MD["governance/*.md\n거버넌스"]
        MIG_MD["migration/*.md\n마이그레이션"]
    end

    subgraph GRID["AG Grid (인터랙티브 그리드)"]
        INST["catalog/instances.md\nag-grid-instances.js"]
        COLS["catalog/columns.md\nag-grid-catalog.js"]
        GLOSS["standards-dict/glossary.md\nag-grid-glossary.js"]
        DOM["standards-dict/domains.md\nag-grid-domain.js"]
        CODE["standards-dict/codes.md\nag-grid-codes.js"]
        ABBR["standards-dict/abbreviations.md\nag-grid-abbreviations.js"]
    end

    subgraph JSON["assets/data/ (JSON)"]
        J1["catalog-instances.json"]
        J2["catalog-columns.json"]
        J3["glossary-terms.json"]
        J4["domain-types.json"]
        J5["code-dictionary.json"]
    end
