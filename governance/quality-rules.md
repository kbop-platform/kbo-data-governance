...
> 📋 문서 성격: 신규 시스템 품질 규칙(안) · 현행 S2i 참고 포함
...
→ 참고: [도메인 타입 정의](../standards-dict/domains.md) - 타입별 허용 범위
...
→ 참고: [도메인 타입 §4](../standards-dict/domains.md) - NULL 처리 정책
...
→ 참고: [코드 사전](../standards-dict/codes.md) - 전체 코드값 정의
...
→ 참고: [도메인 타입 §2.5](../standards-dict/domains.md) - rate 타입 정의
...
→ 참고: [도메인 타입 §3](../standards-dict/domains.md) - 인코딩 정책
...
→ 참고: [코드 사전 부록C](../standards-dict/codes.md) - 미확인 코드 전체 목록
→ 참고: [변경 절차 §4.2](./change-process.md#42-c3) - 미확인 코드 확정 절차

---

## 10. 신선도(Freshness) SLA

데이터 유형별 최신성 보장 기준:

| 테이블 | 갱신 SLA | 측정 기준 | 알림 조건 |
|--------|---------|----------|----------|
| GAMEINFO, Score | 경기 종료 후 30분 내 | DB2 INSERT 시각 | 경기 종료 + 30분 초과 시 |
| Hitter, Pitcher | 경기 종료 후 30분 내 | DB2 INSERT 시각 | 경기 종료 + 30분 초과 시 |
| IE_LiveText, IE_BallCount | 실시간 (< 5초) | S2i 전송 → DB2 저장 지연 | 5초 초과 시 |
| BatTotal, PitTotal | D+1 (전일 경기 반영) | 집계 완료 시각 | 익일 09:00 미반영 시 |
| SEASON_PLAYER_* | D+1 | 집계 완료 시각 | 익일 09:00 미반영 시 |
| TeamRank | D+1 | 집계 완료 시각 | 익일 09:00 미반영 시 |
| person | 시즌 개시 전 | 마지막 갱신일 | 시즌 개시 7일 전 미갱신 시 |
| TEAM, STADIUM | 연 1회 (시즌 전) | 마지막 갱신일 | 시즌 개시 30일 전 미갱신 시 |
| KBO_schedule | 시즌 전 일괄 | 마지막 갱신일 | 시즌 개시 60일 전 미등록 시 |

## 11. 이상치 탐지 기준

데이터 값의 논리적 범위를 벗어나는 경우 경고를 발생시킨다:

| 테이블.컬럼 | 정상 범위 | WARNING 조건 | ERROR 조건 |
|------------|----------|-------------|-----------|
| GAMEINFO.CROWD | 0 ~ 30,000 | > 30,000 | > 40,000 또는 < 0 |
| GAMEINFO.TEMP | 0 ~ 400 (×10) | > 380 (38℃) | > 450 (45℃) 또는 < -100 (-10℃) |
| GAMEINFO.MOIS | 0 ~ 100 | > 95 | > 100 또는 < 0 |
| GAMEINFO.WINS | 0 ~ 15 | > 12 | > 20 |
| Hitter.HIT | 0 ~ 10 | > 6 (1경기) | > 10 |
| Pitcher.INN2 | 0 ~ 27 (9이닝×3) | > 27 | > 36 (12이닝) |
| Score 이닝 점수 | -1 ~ 30 | > 15 (1이닝) | > 30 |
| BatTotal.AVG | 0.000 ~ 1.000 | > 0.500 | > 1.000 또는 < 0.000 |
| person.BIRTH | 19500101 ~ 20101231 | 범위 외 | NULL 또는 형식 불일치 |

> `-1`은 Score 테이블에서 미진행 이닝을 의미하는 센티널 값으로, 이상치가 아님.

## 12. 품질 KPI 대시보드 (모형)

| KPI | 산출 공식 | 목표 | 측정 주기 |
|-----|----------|------|----------|
| PK 무결성 | (PK NULL 없는 행 수 / 전체 행 수) × 100 | 100% | 일간 |
| FK 정합성 | (FK 매칭 행 수 / 전체 FK 참조 행 수) × 100 | ≥ 99.9% | 일간 |
| 필수값 완결성 | (NOT NULL 컬럼 중 값 존재 행 수 / 전체) × 100 | ≥ 99.5% | 일간 |
| 코드 유효성 | (유효 코드값 행 수 / 전체 코드값 행 수) × 100 | ≥ 99.0% | 주간 |
| 신선도 준수율 | (SLA 내 갱신된 테이블 수 / 전체 대상) × 100 | ≥ 95% | 일간 |
| 이상치 비율 | (WARNING + ERROR 건수 / 전체 행 수) × 100 | ≤ 0.1% | 일간 |
| 데이터 정합성 | (DB1과 DB2 행 수 일치 테이블 / 전체) × 100 | 100% | 주간 |
